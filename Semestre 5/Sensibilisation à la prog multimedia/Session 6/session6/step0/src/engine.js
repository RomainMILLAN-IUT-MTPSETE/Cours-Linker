...
function init(htmlCanvasID) {
  ...
  const colorLoc = renderSystem.init(mGL);
  ecs.addSystem((entities) =>
    inputSystem.inputSystem(entities, ecs.components)
  );
  ecs.addSystem((entities) =>
    physicsSystem.physicsSystem(entities, ecs.components)
  );
  ecs.addSystem((entities) =>
    collisionSystem.collisionSystem(entities, ecs.components, ecs)
  );
  //TODO
  //ecs.addSystem(entities => cssRenderSystem.cssRenderSystem(entities, ecs.components, gameContainer));
  ecs.addSystem((entities) =>
    renderSystem.webGLRenderSystem(entities, ecs.components, mGL, colorLoc)
  );
  //---
  ecs.addSystem((entities) =>
    gameoverSystem.gameoverSystem(entities, ecs.components, ecs, 25)
  );
  ecs.addSystem((entities) =>
    gameoverSystem.ballUnderRaquetteSystem(entities, ecs.components, ecs)
  );
  ecs.addSystem((entities) =>
    hudSystem.hudSystem(entities, ecs.components, gameContainerHUD)
  );
