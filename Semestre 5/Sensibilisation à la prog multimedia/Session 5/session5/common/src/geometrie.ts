...
// Notion de Ligne dans le Plan 2D
class Line3D extends AbstractLine {
    constructor(public point: Point3D, public direction: Vector) {
        super();
    }

    // Équation paramétrique de la ligne
    parametricEquation(t: number): Point3D {
        return new Point3D(
            this.point.x + t * this.direction.x,
            this.point.y + t * this.direction.y,
    ...
    // Relation d'incidence avec un point 2D
    isIncidentWith(point: Point3D): boolean {
        // (à implémenter selon les besoins)
        return false;
    }
}
