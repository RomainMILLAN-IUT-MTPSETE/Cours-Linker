...
def estPresquePremier(n):
    for a in range(2, 5):
        if ExponentielModulaire(a, n - 1, n) != 1:
            return 0
    return 1
...
print(estPresquePremier(162))


# 2°)
def genNbPresquePremierBeetwen2AndN(b):
    i = randint(2, (2 ** b))
    while estPresquePremier(i) != 1:
        i = randint(2, (2 ** b))
    return i


print("Question n°2 ", genNbPresquePremierBeetwen2AndN(12))

# 3°) Il faut en moyenne 10 entiers

# 4°) On peut le rendre plus efficace en générant que des nombres impair & en rendant la fonction estPresquePremier plus efficace
...
# 1°)
def genCleRSA(nbBit):
    p = genNbPresquePremierBeetwen2AndN(nbBit)
    q = genNbPresquePremierBeetwen2AndN(nbBit)
    print(p)
    print(q)
    N = p * q
    e = genNbPresquePremierBeetwen2AndN(nbBit)

    a, d, c = euclideEtendue(e, ((p - 1) * (q - 1)))
...
N, e, d = genCleRSA(1000)
print("Génération clé RSA ", N, e, d)
chf = E(1134422666777244, (N, e))
print("Chiffrement: ", chf)
df = D(chf, (N, d))
print("Déchiffrement: ", df)
