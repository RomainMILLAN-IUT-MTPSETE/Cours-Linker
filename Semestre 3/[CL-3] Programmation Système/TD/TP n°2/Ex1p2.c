...
int main(int argc, char **argv)
{
    ...
    if(nbThread > TAILLE_FILE){
        nbThread = TAILLE_FILE;
    }

    pthread_t* thread_calcul_id = malloc(nbThread*sizeof(pthread_t));
    int taille_par_thread = (TAILLE_FILE/nbThread)+1;

    argument_calcul_t* parametre_calcul = malloc(nbThread*sizeof(argument_calcul_t));

    for(int i=0; i<nbThread; i++){
        parametre_calcul[i].index_debut = i * taille_par_thread;
        parametre_calcul[i].index_fin = parametre_calcul[i].index_debut + taille_par_thread;
        parametre_calcul[i].step = 1;

        if(parametre_calcul[i].index_fin > TAILLE_FILE){
            parametre_calcul[i].index_fin = TAILLE_FILE;
        }

        pthread_create(&thread_calcul_id[i], NULL,
                       activite_calculateur, &parametre_calcul[i]);
    }
    ...
    for(int i=0; i<nbThread; i++){
        pthread_join(thread_calcul_id[i], NULL);
